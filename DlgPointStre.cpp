...
IMPLEMENT_DYNAMIC(DlgPointStre, CDialogEx)

DlgPointStre::DlgPointStre(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DLG_PointStre, pParent)
	, m_bX1(0)
	, m_bY1(0)
	, m_bX2(0)
	, m_bY2(0)
...
DlgPointStre::~DlgPointStre()
{
}

void DlgPointStre::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(DlgPointStre, CDialogEx)
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_WM_PAINT()
	...
	ON_BN_CLICKED(IDPreview, &DlgPointStre::OnBnClickedPreview)
	ON_BN_CLICKED(IDRecover, &DlgPointStre::OnBnClickedRecover)
END_MESSAGE_MAP()


// DlgPointStre 消息处理程序

BOOL DlgPointStre::OnInitDialog()
{
...
void DlgPointStre::OnKillfocusEditX1()
{
	// 更新
	UpdateData(TRUE);
...
void DlgPointStre::OnKillfocusEditX2()
{
	// 更新
	UpdateData(TRUE);
...
void DlgPointStre::OnKillfocusEditY1()
{
	// 更新
	UpdateData(TRUE);
...
void DlgPointStre::OnKillfocusEditY2()
{
	// 更新
	UpdateData(TRUE);
...
void DlgPointStre::OnLButtonDown(UINT nFlags, CPoint point)
{
	// 当用户单击鼠标左键开始拖动
...
void DlgPointStre::OnMouseMove(UINT nFlags, CPoint point)
{
	// 判断当前光标是否在绘制区域
	if (m_MouseRect.PtInRect(point))
	{
...
void DlgPointStre::OnLButtonUp(UINT nFlags, CPoint point)
{
	// 当用户释放鼠标左键停止拖动
	if (m_iIsDraging != 0)
	{
...
void DlgPointStre::OnPaint()
{
	...
	// 绘制坐标值
	str.Format(_T("(%d, %d)"), m_bX1, m_bY1);
	pDC->TextOut(m_bX1 + 10, 281 - m_bY1, str);
	str.Format(_T("(%d, %d)"), m_bX2, m_bY2);
...
void DlgPointStre::OnBnClickedPreview()
{
	// TODO: 在此添加控件通知处理程序代码
...
void DlgPointStre::OnBnClickedRecover()
{
	// TODO: 在此添加控件通知处理程序代码
	pDoc->OnImgrestore();
}
